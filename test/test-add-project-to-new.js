...
async function testAddProjectToNew() {
  try {
    ...
    // Check the new hackathon (ID 3)
    console.log("\n=== Checking New Hackathon (ID 3) ===");
    const hackathonData = await contract.hackathons(3);
    ...
    // Try to add a project to the new hackathon
    console.log("\n=== Attempting to Add Project to New Hackathon ===");
    try {
      const tx = await contract.registerProject(
        3,
        "Test Project in New Hackathon",
        "A test project in the newly created hackathon"
      );
      ...
      const updatedData = await contract.hackathons(3);
      console.log("Updated project count:", updatedData[10].toString());
